...
function validateAuth0JWT(): Array<(req: Request, res: Response, next: NextFunction) => void> {
    return [
        arrayPolyfill,
        ...
        async function (req: Request, res: Response, next: NextFunction): Promise<void> {
            try {
                if (req.auth?.sub) {
                    ...
                    return sendResponse(res, {}, 'Invalid authentication token', RESPONSE_FAILURE, RESPONSE_CODE.UNAUTHORISED);
                }
